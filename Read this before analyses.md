...
#### Specific to Questionnaire on paper
...
For digital Questionnaire limits for valid values set are defined by the questionnaire itself. I.e. for question where only one answer is intended the quesionnaire use a radio button that only let the user select one option of a collection of options. 
### Use of data
